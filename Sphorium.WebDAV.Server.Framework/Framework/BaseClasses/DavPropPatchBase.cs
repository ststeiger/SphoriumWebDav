		...
		private int DavPropPatchBase_InternalProcessDavRequest(object sender, EventArgs e)
		{
			...
			else
			{
				using (Stream _responseStream = new MemoryStream())
				{
                    ...
                    System.Collections.Generic.List<DavProperty> ls = new System.Collections.Generic.List<DavProperty>();

                    foreach (DavProperty thisProperty in this.RequestDeleteProperties)
                    {
                        ls.Add(thisProperty);
                    }

                    foreach (DavProperty thisProperty in this.RequestModifyProperties)
                    {
                        ls.Add(thisProperty);
                    }

                    foreach (var _prop in ls)
                    {
						...
						// (
							...
							//{
                                ...
                                foreach(DavProperty p in this.PatchedResource.CustomProperties)
                                {
                                    if(p.Name == _prop.Name && p.Namespace == _prop.Namespace)
                                        _count++;
                                }
